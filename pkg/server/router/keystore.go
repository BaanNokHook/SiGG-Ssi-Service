...
func (sk StoreKeyRequest) ToServiceRequest() (*keystore.StoreKeyRequest, error) {
	// make sure we can decode and re-endcode the key before storing it
...
func (ksr *KeyStoreRouter) GetKeyDetails(ctx context.Context, w http.ResponseWriter, _ *http.Request) error {
	...
	if id == nil {
		errMsg := "cannot get key details with ID parameter"
		logrus.Error(errMsg)
		return framework.NewRequestErrorMsg(errMsg, http.StatusBadRequest)
	}

	getKeyDetails, err := ksr.service.GetKeyDetails(keystore.GetKeyDetailsRequest{ID: *id})
	if err != nil {
	...
	resp := GetKeyDetailsResponse{
		ID:         getKeyDetails.ID,
		Type:       getKeyDetails.Type,
		Controller: getKeyDetails.Controller,
		CreatedAt:  getKeyDetails.CreatedAt,
	}
